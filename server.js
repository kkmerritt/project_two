...
// // an individual patch udpate of 1x post
// server.patch('/postdir/:id', function(req, res){
//   var postOptions = req.body.post;
//   Post.findByIdAndUpdate(req.params.id, postOptions, { $push:true}, function(err, updatedPost){
//     if (err){console.log("FIND DATABASE ERROR. for fuck's sake", err);   //don't fix this later
//     } else {res.redirect(302, '/postdir/' + updatedPost._id)}
//     });
//   });
  // an individual patch udpate of 1x post
  ...
  server.post('/postdir/:id/comment', function(req, res){
    ...
    Post.findById(req.params.id, function (err, foundPost) {
        if (err) { console.log("ERRbody in the club..", err) }
          ...
          foundPost.comment.push(req.body.post.comment) // english what?
